...
module KubernetesDeploy
  class Runner
    ...
    def prune_whitelist
      resources = DiscoverableResource.all.select { |res| res.prunable? }
      identities = resources.map { |res| res.identity }
      PRUNE_WHITELIST + identities
    end
    
    def predeploy_sequence
      resources = DiscoverableResource.all.select { |res| res.predeploy? }
      identities = resources.map { |res| res.identity }
