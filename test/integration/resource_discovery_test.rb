...
class ResourceDiscoveryTest < KubernetesDeploy::IntegrationTest
  ...
  def kubectl
    @kubectl ||= KubernetesDeploy::Kubectl.new(namespace: @namespace,
      ...
      logger: KubernetesDeploy::FormattedLogger.build(
        @namespace, 
  ...
  def cleanup(*resources)
    resources.each do |res|
      _, _err, st = kubectl.run("delete", res, "--all")
      flunk(_err) unless st.success?
    end
  end

  def has_tpr_support?
    _, _err, st = kubectl.run("get", "thirdpartyresources")
    st.success?
  end
  ...
  def test_prunable_tpr
    ...
    refute_logs_match("Don't know how to monitor resources of type Gizmo. Assuming Gizmo/my-first-gizmo deployed successfully")
  ensure
    cleanup('thirdpartyresources')
  end

  def test_non_prunable_crd_no_predeploy
    assert_deploy_success(deploy_fixtures("resource-discovery/definitions", subset: ["crd_non_prunable_no_predeploy.yml"]))
    ...
    refute_logs_match("Don't know how to monitor resources of type Widget. Assuming Widget/my-first-widget deployed successfully")
